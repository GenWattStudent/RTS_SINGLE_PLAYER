	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float simplePerlin2D321 = snoise( (v.positionOS*1.0 + ( _TimeParameters.x * _WindMovement )).xy*_WindDensity );
				simplePerlin2D321 = simplePerlin2D321*0.5 + 0.5;
				float4 appendResult329 = (float4(( ( ( ( simplePerlin2D321 - 0.5 ) / 10.0 ) * _WindStrength ) + v.positionOS.x ) , v.positionOS.y , v.positionOS.z , 1.0));
				float4 lerpResult330 = lerp( v.positionOS , appendResult329 , ( v.positionOS.y * 2.0 ));
				...
				#else
				float4 staticSwitch333 = v.positionOS;
				#endif
						 ...
						 ) : SV_Target
			{
				...
				float2 texCoord204 = IN.ase_texcoord8.xy * float2( 1,1 ) + temp_cast_1;
				float2 temp_cast_2 = (0.0).xx;
				float2 texCoord235 = IN.ase_texcoord8.xy * float2( 1,1 ) + temp_cast_2;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float grayscale313 = dot(tex2DNode2.rgb, float3(0.299,0.587,0.114));
				float2 temp_cast_1 = (0.5).xx;
				float2 texCoord204 = IN.ase_texcoord4.xy * float2( 1,1 ) + temp_cast_1;
				float2 temp_cast_2 = (0.0).xx;
				float2 texCoord235 = IN.ase_texcoord4.xy * float2( 1,1 ) + temp_cast_2;
				float temp_output_238_0 = ( step( texCoord204.x , 1.0 ) + step( texCoord235.y , 0.5 ) );
				float clampResult248 = clamp( ( 1.0 - temp_output_238_0 ) , 0.0 , 1.0 );
				float FLOWERMASK395 = clampResult248;
				float4 temp_cast_3 = (( grayscale313 * FLOWERMASK395 )).xxxx;
...
ASEEND*/
//CHKSM=9658BECADB9F3EE3B3E4F14EA2235A99B9D72F72